...
if __name__ == "__main__":
    ...
    class SentimentInference:
        def __init__(self, config_path):
            ...
            self.model_hf_repo_id = config_data['model']['name'] 
            self.tokenizer_name_or_path = config_data['model'].get('tokenizer_name_or_path', self.model_hf_repo_id)
            
            # Get local model path from 'inference' section
            self.local_model_weights_path = config_data.get('inference', {}).get('model_path', None)
            # Determine if we should load from local_pt based on presence of local_model_weights_path
            self.load_from_local_pt = bool(self.local_model_weights_path)
            ...
            try:
                if self.load_from_local_pt and self.local_model_weights_path:
                    ...
                    # Step 1: Load the base configuration from model_hf_repo_id
                    # This provides the architecture details if not saved with the checkpoint.
                    print(f"Loading base configuration from: {self.model_hf_repo_id}")
                    self.config = AutoConfig.from_pretrained(
                        self.model_hf_repo_id, 
                        trust_remote_code=self.trust_remote_code_for_config
                    )
                    
                    # CRITICAL: Override num_labels to match the checkpoint
                    self.config.num_labels = 1
                    # Also ensure other critical params from training are set if not in base config
                    # These should ideally be saved with the model's config if possible, or loaded from the training config
                    self.config.pooling_strategy = self.config_data.get('model', {}).get('pooling_strategy', 'cls')
                    self.config.num_weighted_layers = self.config_data.get('model', {}).get('num_weighted_layers', 4)
                    # The loss_function config is part of the model's init in src/models.py, so ensure it's on the config
                    self.config.loss_function = self.config_data.get('model', {}).get('loss_function', {'name': 'SentimentWeightedLoss', 'params': {}})

                    # Step 2: Instantiate the model architecture using this configuration
                    print("Instantiating model structure with loaded configuration...")
                    self.model = ModernBertForSentiment(config=self.config)
                    
                    # Step 3: Load the state dictionary from the .pt file
                    print(f"Loading state dict from checkpoint: {self.local_model_weights_path}")
                    checkpoint = torch.load(self.local_model_weights_path, map_location=self.device)
                    
                    # Extract model_state_dict (train.py saves a dict with 'model_state_dict')
                    if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
                        model_state_dict = checkpoint['model_state_dict']
                        print("Extracted 'model_state_dict' from checkpoint dictionary.")
                    elif isinstance(checkpoint, dict) and all(k.startswith('bert.') or k.startswith('classifier.') for k in checkpoint.keys()):
                        # Heuristic: if all top-level keys look like model layers, it might be a raw state_dict
                        print("Checkpoint appears to be a raw state_dict (no 'model_state_dict' key, but keys match model layers).")
                        model_state_dict = checkpoint
                    else:
                        # Fallback: assume the checkpoint object IS the state_dict itself (e.g. if saved with torch.save(model.state_dict(), PATH))
                        print("Warning: Checkpoint is not a dict with 'model_state_dict' key, and keys don't obviously match model layers. Assuming the loaded object IS the state_dict.")
                        model_state_dict = checkpoint
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of samples to evaluate from the dataset. Default is all samples."
    )
    ...
    if args.num_samples is not None and args.num_samples > 0:
        print(f"Evaluating on {args.num_samples} samples from the test set.")
        try:
            imdb_dataset_test = imdb_dataset_test.select(range(args.num_samples))
        except IndexError as e:
            print(f"Error selecting samples: {e}. Ensure num_samples is not greater than dataset size ({len(imdb_dataset_test)}).")
            exit()
        except Exception as e:
            print(f"An unexpected error occurred while selecting samples: {e}")
            exit()
    else:
        print(f"Evaluating on the full test set ({len(imdb_dataset_test)} samples).")
